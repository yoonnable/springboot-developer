...
public class OAuth2SuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    private void saveRefreshToken(Long userId, String newRefreshToken) {
        RefreshToken refreshToken = refreshTokenRepository.findByUserId(userId)
                ...
                .orElse(new RefreshToken(userId, newRefreshToken)); // 리프레시 토큰을 전달 받아

        refreshTokenRepository.save(refreshToken); // 리프레시 토큰을 저장
    }
    ...
    private void clearAuthenticationAttributes(HttpServletRequest request, HttpServletResponse response) {
        super.clearAuthenticationAttributes(request); // 기본적으로 제공하는 메서드는 그대로 호출
        authorizationRequestRepository.removeAuthorizationRequestCookie(request, response); // 이 메서드를 추가로 호출해 OAuth 인증을 위해 저장된 정보도 삭제
    }
